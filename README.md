...
Official Python client library for the [UniBee Billing API](https://docs.unibee.dev).

UniBee is an open-source billing software designed for SaaS businesses, offering subscription management, invoicing, payment processing, and more.

## Features

- **Subscription Management**: Create, update, cancel, and manage subscriptions
- **Invoice Management**: Generate, send, and track invoices
- **Payment Processing**: Handle payments through multiple gateways (Stripe, PayPal, and more)
- **Product & Plan Management**: Create and manage products and pricing plans
- **User Management**: Manage customer accounts and profiles
- **Discount Codes**: Create and apply promotional discounts
- **Webhooks**: Configure webhook endpoints for real-time event notifications
- **Metrics & Billing**: Track usage-based billing metrics
...
configuration = openapi_client.Configuration(
    host="https://api.unibee.dev",  # Production API
    # host="https://api-sandbox.unibee.top",  # Sandbox (for testing)
...
# Configure SDK
configuration = openapi_client.Configuration(
    host="https://api.unibee.dev",
    access_token="your_api_token_here"
)

with openapi_client.ApiClient(configuration) as api_client:
    # Create Subscription API instance
    subscription_api = openapi_client.Subscription(api_client)
    
    try:
        # Get list of subscriptions
        response = subscription_api.subscription_list_get()
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        ...
        preview = subscription_api.subscription_create_preview_post(
            openapi_client.UnibeeApiMerchantSubscriptionCreatePreviewReq(
                plan_id=123,
                user_id=456,
                gateway_id=1
            )
        )
        print("Subscription Preview:", preview)
    except ApiException as e:
        print(f"API Error: {e}")
```
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Get product list
        products = product_api.product_list_get(page=0, count=20)
...
with openapi_client.ApiClient(configuration) as api_client:
    discount_api = openapi_client.Discount(api_client)
    
    try:
        # Create a 20% discount code
        discount = discount_api.discount_new_post(
            code="SAVE20",
            name="20% Off",
            billing_type=2,  # Recurring
            discount_type=1,  # Percentage
            discount_percentage=20,
            quantity=100
        )
        print("Created Discount:", discount)
    except ApiException as e:
        print(f"API Error: {e}")
```
...
with openapi_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Get user profile
        user = user_api.user_get_get(user_id=123)
...
| Module | Description |
|--------|-------------|
| `Auth` | Authentication and login operations |
| `Balance` | Query merchant and user balances |
| `Checkout` | Create and manage checkout sessions |
| `Credit` | Manage user credits and promo credits |
| `Discount` | Create and manage discount codes |
| `Email` | Email gateway and template management |
| `EmailTemplate` | Email template configuration |
| `File` | File upload operations |
| `Gateway` | Payment gateway configuration |
| `Invoice` | Invoice management |
| `Member` | Merchant member management |
| `Metric` | Billable metrics configuration |
| `Payment` | Payment operations and refunds |
| `Plan` | Subscription plan management |
| `Product` | Product management |
| `Profile` | Merchant profile management |
| `Role` | Role and permission management |
| `Search` | Search functionality |
| `Session` | User portal session management |
| `Subscription` | Subscription lifecycle management |
| `SubscriptionNote` | Subscription notes |
| `SubscriptionPendingUpdate` | Pending subscription updates |
| `SubscriptionTimeline` | Subscription timeline events |
| `User` | User/customer management |
| `UserMetric` | User metric queries |
| `Vat` | VAT/tax configuration |
| `Webhook` | Webhook endpoint management |

## Error Handling

The SDK raises `ApiException` for API errors. You can catch and handle these exceptions:
...
```python
configuration = openapi_client.Configuration(
    host="https://api.unibee.dev",     # API base URL
    access_token="your_token",          # Bearer token for authentication
    debug=False,                        # Enable debug logging
    verify_ssl=True,                    # SSL certificate verification
    ssl_ca_cert=None,                   # Custom CA certificate path
...
```python
configuration = openapi_client.Configuration(
    host="https://api-sandbox.unibee.top",
    access_token="your_sandbox_token"
)
```

See [Testing Cards (Sandbox)](https://docs.unibee.dev/documentation/quick-start/testing-cards-sandbox) for test card numbers.

## Documentation

- [UniBee Documentation](https://docs.unibee.dev)
- [API Reference](https://docs.unibee.dev/api-reference)
- [Integration Workflow Guide](https://docs.unibee.dev/documentation/quick-start/integration-workflow-guide)

## Documentation for API Endpoints

All URIs are relative to *https://api.unibee.dev*
...
*Role* | [**role_delete_post**](docs/Role.md#role_delete_post) | **POST** /merchant/role/delete | Delete Merchant Role
...
- [GitHub Issues](https://github.com/UniBee-Billing/unibee-python-client/issues)
- [UniBee Community](https://unibee.dev/contact)
